...
- **`lease`** - provider, term, rate, residual, FBT status, `fortnightlyPreTaxAud` breakdown, net salary impact, `annualAud` figures. `vehicleReturnOption: false` (residual must be paid to own). Optional `leaseVsLoanAdvantageAudPerYr` - the fixed Layer 3 annual figure shown on the Dashboard: the after-tax advantage of the novated lease over a 7% private car loan, per the Jul-2026 Layer 3 re-audit. When absent the app falls back to its built-in constant ($3,275/yr, `data/compute.js:layer3AnnualAud`). The legacy `taxSavingAudPerYr` key (the superseded May-2026 "tax saving" figure, $5,378 on a different basis) is deliberately **ignored** - honoring it would pin pre-re-audit backups to a replaced number.
