...
export default function RoiLayers({ state }) {
  ...
  return (
    <>
      <p className="small">
        Running totals covering <strong>{period}</strong> ({totalMonths} month{totalMonths === 1 ? '' : 's'}
        {' '}of data) — kept separate so nothing is double-counted.
      </p>
      <div className="grid cols-3">
        <div className="metric">
          <div className="label">Layer 1 — Solar + Battery</div>
          <div className="value green">{money(layer1)}</div>
          <div className="sub">Total for {period}</div>
        ...
        <div className="metric">
          ...
          <p className="explainer">
            ...
            minus what it actually costs to charge the EV. (For reference, a full
            year of that fuel+servicing budget is {money(layer2AnnualScope)}; the
            number above is the real running total for {period}, not a projection.)
          </p>
        </div>
        <div className="metric">
      ...
      <h3 style={{ marginTop: '1rem' }}>Combined household benefit for {period} (breakdown, not blended)</h3>
      ...
      <p className="small">
        Layers are kept separate to avoid double-counting: 1 &amp; 2 are running totals
